...
const categories = ['relevance', 'accuracy', 'completeness', 'freshness', 'actionability'] as const;
...
function calculateScore(scores: Record<string, number>): number {
  return Math.round(categories.reduce((sum, cat) => sum + (scores[cat] || 0), 0) / 5);
}
...
const judgeResults = readFileSync(join(RESULTS_DIR, 'judge-result.jsonl'), 'utf-8')
  .trim()
  .split('\n')
  .filter(Boolean)
  .map(line => JSON.parse(line));

const kirhaResults = readFileSync(join(RESULTS_DIR, 'kirha-result.jsonl'), 'utf-8')
  .trim()
  .split('\n')
  .filter(Boolean)
  .map(line => JSON.parse(line));

const websearchResults = readFileSync(join(RESULTS_DIR, 'exa-result.jsonl'), 'utf-8')
  .trim()
  .split('\n')
  .filter(Boolean)
  .map(line => JSON.parse(line));
...
const results = judgeResults.map(judge => {
  ...
  const websearch = websearchResults.find((w: { id: number }) => w.id === judge.id);
...
const totalWebsearchTokens = results.reduce((sum, r) => sum + r.websearch.tokens, 0);
const tokenSavingsPercent = totalWebsearchTokens > 0
  ? Math.round((1 - totalKirhaTokens / totalWebsearchTokens) * 100)
  : 0;

const aggregateScores = {
  kirha: { relevance: 0, accuracy: 0, completeness: 0, freshness: 0, actionability: 0, score: 0 },
  websearch: { relevance: 0, accuracy: 0, completeness: 0, freshness: 0, actionability: 0, score: 0 },
};
...
categories.forEach(cat => {
  aggregateScores.kirha[cat] = Math.round(aggregateScores.kirha[cat] / results.length);
  aggregateScores.websearch[cat] = Math.round(aggregateScores.websearch[cat] / results.length);
});
...
const outputPath = join(import.meta.dirname, '..', 'public', 'benchmark-results.json');
