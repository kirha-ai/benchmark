...
const sourceConfig = {
  ...
  websearch: { label: 'Web Search', color: '#f97316', rawDataTitle: 'Web Search Raw Data' },
} as const
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2 flex-wrap">
        ...
        {isWinner && (
          <Badge className="text-xs" style={{ backgroundColor: config.color }}>Winner</Badge>
        )}
        <span className="ml-auto">
          <span className={`text-xl sm:text-2xl font-bold ${!isKirha ? 'text-muted-foreground' : ''}`}>
            {data.score}
          </span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <h4 className="font-medium text-xs sm:text-sm">{config.label} Output</h4>
        {data.rawData && (
          <Button
            variant="outline"
            size="sm"
...
export function ResultDetailModal({ result, onClose, onShowRawData }: ResultDetailModalProps) {
      ...
      <DialogContent className="w-[calc(100%-2rem)] sm:w-full md:!max-w-[1100px] max-h-[95vh] sm:max-h-[95vh] overflow-y-auto">
        <DialogHeader className="pb-2 sm:pb-4">
          <div className="flex items-center gap-2">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">Query #{result.id}</p>
            {result.vertical && (
              <Badge variant="outline" className="px-1.5 py-0 capitalize">{result.vertical}</Badge>
            )}
          </div>
          ...
          {result.lastRunDate && (
            <p className="text-xs text-muted-foreground">Run on {result.lastRunDate}</p>
          )}
        </DialogHeader>
        ...
        <div className="px-0 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8">
            <SourceOutputCard
              source="kirha"
              data={result.kirha}
              onShowRawData={() => onShowRawData(sourceConfig.kirha.rawDataTitle, result.kirha.rawData)}
            />
            <SourceOutputCard
              source="websearch"
              data={result.websearch}
              onShowRawData={() => onShowRawData(sourceConfig.websearch.rawDataTitle, result.websearch.rawData)}
            />
          </div>
        </div>
      </DialogContent>
